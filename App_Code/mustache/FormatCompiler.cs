    ...
    public sealed class FormatCompiler
    {
            ...
            string format, int formatIndex)
        {
            while (true)
            {
                ...
                else if (match.Groups["open"].Success || match.ToString().Contains("^"))
                {
                    formatIndex = match.Index + match.Length;
        ...
        private ArgumentCollection getArguments(TagDefinition definition, Match match, List<Context> context)
        {
            ...
            if (match.ToString().Contains("^") && parameters.Count > 0)
            {
                try
                {
                    var o = new ArgumentCollection();
                    string s = match.Value.Trim();
                    string _condition = s.Split('^')[1].Trim();
                    _condition = _condition.Substring(0, _condition.Length - 2).Trim();
                    string[] a = s.Split('^')[0].Trim().Split(' ');
                    string _collection_name = a[a.Length - 1];

                    o.AddArgument(parameters[0], new PlaceholderArgument(_collection_name));
                    o.AddArgument(new TagParameter("___condition") { DefaultValue = null, IsRequired = false },
                        new StringArgument(_condition));
                    return o;
                }
                catch {
                    throw new Exception();
